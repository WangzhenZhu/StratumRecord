...
@interface SRFeedbackViewController () <UITextViewDelegate, UITextFieldDelegate>
...
- (void)srm_setupUI {
    ...
    // 反馈类型容器
    self.typeContainer = [[UIView alloc] init];
    ...
    for (NSInteger i = 0; i < types.count; i++) {
        ...
        [self.typeContainer addSubview:button];
...
- (void)srm_setupConstraints {
    ...
    [self.typeContainer mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    for (UIButton *button in self.typeButtons) {
        [button mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(self.typeContainer);
            make.height.mas_equalTo(36);
            if (prevButton) {
                make.left.equalTo(prevButton.mas_right).offset(10);
                make.width.equalTo(prevButton);
            } else {
                make.left.equalTo(self.typeContainer);
            }
            if (button == self.typeButtons.lastObject) {
                make.right.equalTo(self.typeContainer);
            }
        }];
        prevButton = button;
    }
    ...
    [self.feedbackLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.typeContainer.mas_bottom).offset(24);
        make.left.right.equalTo(self.contentView).inset(20);
    }];
